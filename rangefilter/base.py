...
class IntegerRangeFilter(admin.filters.FieldListFilter):
    ...
    def _make_query_filter(self, request, validated_data):
        query_params = {}
        query_value_gte = validated_data.get(self.lookup_kwarg_gte, None)
        query_value_lte = validated_data.get(self.lookup_kwarg_lte, None)

        if query_value_gte is not None:
            query_params['{0}__gte'.format(self.field_path)] = query_value_gte
        if query_value_lte is not None:
            query_params['{0}__lte'.format(self.field_path)] = query_value_lte
    ...
    def get_template(self):
        return 'rangefilter/query_filter.html'
    ...
    def _get_form_fields(self):
        return OrderedDict((
            (self.lookup_kwarg_gte, forms.IntegerField(
                label='',
                widget=AdminIntegerFieldWidget(attrs={'placeholder': _('From')}),
                localize=True,
                required=False
            )),
            (self.lookup_kwarg_lte, forms.IntegerField(
                label='',
                widget=AdminIntegerFieldWidget(attrs={'placeholder': _('To')}),
                localize=True,
                required=False
            )),
        ))
...
class FloatRangeFilter(IntegerRangeFilter):

    def _get_form_fields(self):
        return OrderedDict((
            (self.lookup_kwarg_gte, forms.FloatField(
                label='',
                widget=AdminIntegerFieldWidget(attrs={'placeholder': _('From')}),
                localize=True,
                required=False
            )),
            (self.lookup_kwarg_lte, forms.FloatField(
                label='',
                widget=AdminIntegerFieldWidget(attrs={'placeholder': _('To')}),
                localize=True,
                required=False
...
class DateRangeFilter(IntegerRangeFilter):
